...
Use `"${CLAUDE_PROJECT_DIR}/bin/claude-drop"`. This command is project-scoped;
if that executable is absent, fail closed instead of searching other checkouts.
Never place a secret in command arguments, Bash source, stdout, chat, or your answer.

- Empty arguments, or intent to give you a text value: run `claude-drop request`.
